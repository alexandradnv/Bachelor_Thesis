#!/bin/bash
#SBATCH --job-name=blind-earth-gt-anc-qwen32b
...
#SBATCH --output=logs/blind_earth_gt_anc_qwen32b_%j.log
#SBATCH --error=logs/blind_earth_gt_anc_qwen32b_%j.err
#SBATCH --chdir=/ceph/adinchev/experiment

# Few-shot ground-truth + Mannheim anchor, Qwen/Qwen2.5-32B-Instruct (TP=2).
...
echo "Starting GT+anchor (Qwen2.5-32B-Instruct, TP=2)  at $(date)"
/ceph/adinchev/experiment/.venv/bin/python fewshot_groundtruth_experiment.py \
  --model "Qwen/Qwen2.5-32B-Instruct" --memory-size 3 --resolution 2 \
