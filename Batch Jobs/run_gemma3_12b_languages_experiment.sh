...
#SBATCH --job-name=blind-earth-gemma3-12b-languages
...
#SBATCH --output=logs/blind_earth_gemma3_12b_languages_%j.log
#SBATCH --error=logs/blind_earth_gemma3_12b_languages_%j.err
#SBATCH --chdir=/ceph/adinchev/experiment

# Language & cultural-bias experiment for google/gemma-3-12b-it.
# (Dropped from 27B to 12B because the user QOS caps at 1 GPU and bf16 27B
#  doesn't fit on a single 48 GB card.)
# Runs the blind-earth probe in Russian, Bulgarian, English, German, and Thai.
# Each language uses its own native words for "land" and "water" (configured in
# LANGUAGE_CONFIGS in blind_model_experiment.py).
# The vLLM server is loaded once and reused across all five language runs.
...
echo "Starting Gemma3-12B language-bias experiment..."
...
echo ""
echo "All Gemma3-12B language runs completed at $(date)"
